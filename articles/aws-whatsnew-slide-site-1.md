...
topics: ["aws", "bedrock", "lambda", "slack", "eventbridge"]
published: false
---
...
このPart1では、**AWS What's NewのRSSフィードをLambdaで定期取得し、Amazon Bedrockで要約してSlackに投稿するまで**を解説します。また、Slackのボタンインタラクションを受け取ってBedrock Agentを呼び出す部分も含みます。

:::message
RSS取得・Bedrock要約・Slack通知の基本的な構成は、AWS Samplesとして公開されている **[whats-new-summary-notifier](https://github.com/aws-samples/whats-new-summary-notifier)** を参考にしています。同プロジェクトはCDKでのデプロイに対応しており、SlackとMicrosoft Teams両方に対応しています。本記事ではスライド生成のためのBedrock Agent連携を追加した拡張構成を紹介します。
:::
...
AWS What's NewのRSSフィードを取得し、前回実行時以降の新着エントリを抽出します。

:::message
[whats-new-summary-notifier](https://github.com/aws-samples/whats-new-summary-notifier) では、DynamoDBに処理済みエントリのIDを記録することで重複取得を防止しています。以下のサンプルは時間ベースの簡易フィルタですが、本番運用ではDynamoDBによる管理を推奨します（後述）。
:::
...
def handle_create_slide(ack, body, say):
    ...
    invoke_bedrock_agent(title=title, url=url)
...
## 5. Bedrock Agentの呼び出し

ボタンのインタラクションを受けたら、スライド生成を担うBedrock Agentを呼び出します。詳細はPart2で解説しますが、呼び出し部分のみここに示します。
...
def invoke_bedrock_agent(title: str, url: str):
    response = bedrock_agent.invoke_agent(
        agentId="<Agent ID>",
        agentAliasId="<Agent Alias ID>",
        sessionId=title[:30],  # セッションIDに記事タイトルの先頭を使用
        inputText=f"以下のAWS What's Newをもとに、Marpスライドを作成してください。\nタイトル: {title}\nURL: {url}",
    )
    # レスポンスのストリーム処理
...
Slackはボタンを押してから3秒以内にHTTP 200レスポンスを要求します。Bedrock Agentの呼び出しはそれ以上かかるため、`ack()` を先に返してから非同期で処理を進める設計にしています。
...
今回は「1時間以内に公開されたエントリを取得」という時間ベースのフィルタを使っていますが、DynamoDBなどに処理済みのエントリIDを記録しておくと、より確実に重複を防げます。

[whats-new-summary-notifier](https://github.com/aws-samples/whats-new-summary-notifier) ではDynamoDBでエントリIDを管理する実装が採用されています。Lambda実行タイミングのズレや再実行時の二重投稿を防ぐためにも、本番環境での採用を推奨します。
...
- AWS What's NewのRSSフィード取得
- Claude Haiku 4.5による日本語要約
- Block Kitを使ったボタン付きSlack投稿
- Slack BoltによるボタンインタラクションのハンドリングとBedrock Agent呼び出し

次のPart2では、このBedrock AgentがTavilyで情報を補完しながらMarpスライドを生成する部分を解説します。
