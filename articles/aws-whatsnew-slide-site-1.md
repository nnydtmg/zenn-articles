...
Bedrock Agentの呼び出しは数十秒かかることがあります。ボタンインタラクション受信後すぐに`ack()`でSlackに200を返し、処理は非同期で進める構成が必須でした。`process_before_response=True`とLambdaの非同期invokeを組み合わせることでこれを実現しています。
