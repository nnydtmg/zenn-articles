---
title: "AWS Support - Troubleshooting in the cloud Workshopをやってみた②~EKS編~"
emoji: "🌟"
...
この章では、EKSワークロードに関してのトラブルシューティングを行います。
![](https://storage.googleapis.com/zenn-user-upload/b0accb7e7baf-20240227.png)

必要なリソースは先ほどのECSのリソースと同時に作成されていますので、引き続き実施することが可能です。

## Issue1
kubectlアクセスが権限不足で実行できないので解決していきます。

:::details Information
最近EKS絡みのIAM利用が[改善された](https://aws.amazon.com/jp/about-aws/whats-new/2023/12/amazon-eks-controls-iam-cluster-access-management/)ため、若干異なるかもしれませんが、EKS特有の問題点としてワークショップを進めます。
:::

EKSは作成者のロールがクラスターに付与されるため、EKSの管理者が誰かを確認します。
CloudWatch LogsのLogs Insightsでログをクエリしてみます。デフォルトで検索期間が1時間になっているので、必要に応じて変更します。
...
| limit 50
```
すると、以下のような出力があるので、ロールを確認しておきます。
```
フィールド	値
@ingestionTime 1709006453842
@log xxxxxxxxxxxx:/aws/eks/EKSWorkshop/cluster
@logStream authenticator-3553a6764027dfd2ee50f5c677f3a4fb
@message time="2024-02-27T03:59:38Z" level=info msg="mapping IAM user" groups="[system:masters]" user="arn:aws:sts::xxxxxxxxxxxx:assumed-role/ECS-EKS-EKSWSC9Role-NHhwDOkeoytW/i-0b85ddb07de95cd6a" username=kubernetes-admin
@timestamp 1709006448653
```

先ほどcubectlで使用してるロールを確認してみます。
```
aws sts get-caller-identity
```
