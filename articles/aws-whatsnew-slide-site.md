...
[Slack Bolt: インタラクションハンドラ]
        ↓
[Bedrock Agent: Tavily検索 + Marpスライド生成]
        ↓
[GitHub: MarpスライドをHTMLでエクスポート・保存]
[Cloudflare KV: URL等のメタデータ保存]
        ↓
[Cloudflare Workers (Hono): KVからメタデータ取得 → GitHubのHTMLを配信]
```
...
| サービス | 用途 |
|---|---|
| Cloudflare Workers (Hono) | KVからメタデータ取得・スライドサイト配信 |
...
| Marp | MarkdownからHTMLスライドを生成・GitHub上に保存 |
