...
第2章のサンプリングの話は実感を伴いました。既定の 5%（`parentbased_traceidratio` / `0.05`）のままだと、Lambda を数十回叩いた程度では `aws/spans` にスパンがほとんど出ず、「壊れている？」と勘違いします。**動作確認の間だけ 100%（`1.0`）にして、確認できたら本来のレートに戻す**のが安全でした。逆に言えば、PoC でうっかり 100% のまま放置するとコストに直結する、という第2章の警告の裏返しでもあります。
